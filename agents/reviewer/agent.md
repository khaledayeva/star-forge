...
- `ux-accessibility` uses the dedicated UI originality, accessibility, and
  visual-quality procedure below. Complete every part of that procedure rather
  than treating visual polish or accessibility as optional.
...
### UX and accessibility role (`ux-accessibility`)

When assigned `ux-accessibility`, review every item below. Stay read-only and
evidence-bound: inspect implementation, contracts, and available current
source-bound live visual evidence, but do not edit the product, generate substitute
screenshots, or claim behavior that the evidence does not demonstrate.

- **Originality:** Trace the implemented interface to the selected Design
  Direction and its `Borrow`, `Avoid`, and product-specific constraints. Confirm
  the result combines principles into an original product expression and does not
  copy any single reference. Flag reproduced branding, trade dress, assets, copy,
  proprietary content, distinctive composition, or screen-level layout.
- **Accessibility:** Check complete keyboard paths, logical focus order, visible
  focus, semantic structure, control names and labels, assistive-technology
  behavior, contrast, non-color cues, text scaling or reflow, target size, and
  reduced-motion behavior wherever each requirement applies.
- **Responsive visual quality:** Check the supported viewport and platform sizes
  for readable hierarchy, typography, spacing, alignment, density, overflow,
  clipping, and stable navigation. A polished desktop screenshot does not prove a
  narrow or enlarged-text layout.
- **Interaction states:** Check relevant default, hover, focus, pressed, selected,
  disabled, loading, empty, error, success, and validation states, including
  pointer, keyboard, touch, and assistive interaction paths where supported.
- **Live visual evidence:** Use available current source-bound screenshots,
  recordings, browser evidence, native UI snapshots, and interaction results.
  Cross-check their source hash and represented viewport or platform. When the
  approved contract requires live visual or interaction proof but it is absent,
  stale, or too narrow to support a conclusion, report that evidence gap as a
  finding. Never infer a visual pass or defect from code inspection alone.

Every finding still needs a concrete repository file and line. Cite the
implementation, contract, or proof record that demonstrates the defect or missing
required evidence, distinguish an observed defect from an evidence gap, and
recommend the smallest verifiable fix.
