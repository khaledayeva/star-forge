    ...
    for path in sorted(reviews_root.glob("*/*.json")):
        ...
        if path.name == "merged.json":
            if payload.get("schema") != LEGACY_REVIEW_SCHEMA:
                problems.append(
                    f"{record['path']} does not use legacy review schema "
        ...
        if (
            not isinstance(payload.get("role"), str)
            or not isinstance(payload.get("source_hash"), str)
            or not isinstance(findings, list)
        ):
            problems.append(
                f"{record['path']} is not a readable v0.3 reviewer findings file"
            )
        record.update(
    ...
    if state_root.exists() and not state_root.is_symlink() and state_root.is_dir():
        for path in sorted(state_root.glob("complete-task-*.json")):
            ...
            if payload.get("schema") != LEGACY_COMPLETION_SCHEMA:
                problems.append(
                    f"{_relative(path, root)} does not use legacy completion schema "
    ...
    if proof_path.exists() or proof_path.is_symlink():
        ...
        if problem:
            ...
            if payload.get("schema") != LEGACY_PROOF_SCHEMA:
                problems.append(
                    f"{_relative(proof_path, root)} does not use legacy proof schema "
...
def _state_snapshot(root: Path, problems: list[str]) -> dict[str, Any] | None:
    ...
    if payload.get("schema") != LEGACY_STATE_SCHEMA:
        problems.append(
            f"{_relative(path, root)} does not use legacy state schema "
...
__all__ = [
    ...
    "LEGACY_PROJECT_INSPECTION_SCHEMA",
    "LEGACY_PROOF_SCHEMA",
