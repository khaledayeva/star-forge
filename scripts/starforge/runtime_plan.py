...
from starforge import evidence as project_evidence
...
def task_files(task: dict[str, Any]) -> list[str]:
    raw = task.get("files")
    values = [path for path, _match_kind in project_contracts.task_file_owners(raw)]
        ...
        for line_idx in range(start, end):
            ...
            for key, value in updates.items():
                ...
                if raw_idx is not None:
                    cells[raw_idx] = value
            lines[line_idx] = "| " + " | ".join(map(project_contracts.encode_plan_cell, cells[:len(headers)])) + " |"
...
def append_plan_task(plan_path: Path, row: dict[str, str]) -> bool:
    ...
    lines.insert(end, "| " + " | ".join(map(project_contracts.encode_plan_cell, cells)) + " |")
    write_text(plan_path, "\n".join(lines) + "\n")
    return True
...
def lifecycle_gate_state(project: Path, *, kind: str, required: bool,
    ...
    try:
        contract = read_json(paths[contract_rel])
        envelope = project_evidence.read_envelope(
            paths[evidence_rel],
            project_root=project,
            verify_artifacts=True,
        )
        evidence = project_evidence.lifecycle_payload(envelope, kind)
